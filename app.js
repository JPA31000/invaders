(()=>{
  ...
  function startGame(){
  // Réinitialisation des drapeaux pour un démarrage propre
    overlay.hidden = true;
    tryAgainBtn.hidden = true;
    downloadCsvBtn.hidden = true;
    state.over = false;
    state.pendingGameOver = false;
    state.pendingGameOverUntil = 0;
    state.flashUntil = 0;
    state.showPixelGameOver = false;
    state.running = true;

    // Réinitialisation complète de l'état avant nouvelle partie
    state.over=false; state.pendingGameOver=false; state.pendingGameOverUntil=0; state.running=true;
  ...
  function endGame(){
    // Guard: don't end game if it hasn't started
